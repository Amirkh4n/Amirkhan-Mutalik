...
class Obstacle:  # defines obstacles
    ...
    def draw(self):  # draws obstacle rectangle
        pygame.draw.rect(
            SCREEN,
            YELLOW,
            pygame.Rect(
...
def main():
    ...
    snake = Snake()
    food = Food(5, 5)
    obstacle = Obstacle(10, 10)  # Coordinates of the obstacle
    dx, dy = 0, 0
    prev = 'none'
    score = 0
    level = 0
    ...
    while running:
        ...
        if snake.body[0].x == obstacle.location.x and snake.body[0].y == obstacle.location.y:
            game_over()
        ...
        snake.draw()
        food.draw()
        obstacle.draw()  # Draw the obstacle
        draw_grid()
